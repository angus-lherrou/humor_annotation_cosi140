...
def train_test(d):
    ...
    #unwanted_named_entites = set(['PERCENT','QUANTITY', 'ORDINAL', 'CARDINAL'])
    high_weighted_named_entities = set(['PERSON', 'WORK_OF_ART', 'PRODUCT'])
    mid_weighted_named_entities = set(['GPE', 'ORG', 'EVENT', 'LOC'])
    ...
    for hashtag in d.values():
        for id, tweet, humor in hashtag:
            ...
            doc = nlp(tweet)
            for ent in doc.ents:
                if ent.label_ in high_weighted_named_entities:
                    ents_in_tweet += 0.5
                elif ent.label_ in high_weighted_named_entities:
                    ents_in_tweet += 1
